class Solution {
    ...
    public boolean isSafe(int row, int col, List<StringBuilder> chessboard, int n) { // Queen lies in
                                                                                     // Same Column or Row ?
        for (int i = 0; i < n; i++) {
    ...
    public void recursion(int row, int n, List<StringBuilder> chessboard, List<List<String>> answer) {
        if (row == n) {
            List<String> current = new ArrayList<>();

            for (int i = 0; i < chessboard.size(); i++) {
                current.add(chessboard.get(i).toString());
            }
            answer.add(current);
            return;
        }
